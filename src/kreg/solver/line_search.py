...
def armijo_line_search(
    ...
    def sufficiently_improved(new_val,step):
        return (new_val - val <= -1 * alpha * step * jnp.dot(g, p)) and (not jnp.isnan(new_val))
    
    step = step_init
    new_x = x - step * p
    val, new_val,new_grad = objective(x), objective(new_x), gradient(new_x)
    while (
        (not sufficiently_improved(new_val,step))
         and ((jnp.linalg.norm(new_grad)/jnp.linalg.norm(g))>(1 - grad_decrease))
    ):
        if step <= 1e-15:
            raise RuntimeError(
